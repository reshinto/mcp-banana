...
// It validates all inputs via the security package before forwarding to the Gemini
// client resolved from the cache using the per-request API key injected by middleware.
...
func NewGenerateImageHandler(clientCache *gemini.ClientCache, maxImageBytes int) func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(requestContext context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		result, err := resolvedClient.GenerateImage(requestContext, modelAlias, prompt, gemini.GenerateOptions{
