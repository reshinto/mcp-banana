...
| **Docker: cert mount missing** | `open /certs/fullchain.pem: no such file or directory` or `tls: failed to find any PEM data` | Let's Encrypt stores certs as symlinks (`live/` → `archive/`). Mounting only the `live/` subdirectory breaks the symlinks inside the container | Mount the entire `/etc/letsencrypt` directory: `docker-compose.prod.yml` should have `- /etc/letsencrypt:/etc/letsencrypt:ro`. The prod script handles this automatically |
